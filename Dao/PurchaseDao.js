...
class PurchaseDao {
  ...
  getCustomersByFactoryId(factoryId) {
    const filteredPurchases = this.purchases.filter(
      (purchase) => purchase.factoryId === Number(factoryId)
    );

    const customers = [
      ...new Set(filteredPurchases.map((purchase) => purchase.customerName)),
    ];

    const distinctElements = customers.filter(
  ...
  getCustomersByFactoryIdAccepted(factoryId) {
    const filteredPurchases = this.purchases.filter(
      (purchase) =>
        purchase.factoryId === Number(factoryId) &&
  ...
  getByStatus(status) {
    return this.purchases.filter((purchase) => purchase.status === status);
  }
